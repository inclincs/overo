...
int AACEncode(int argc, char *argv[])
{
...
//        [FOR_DEBUG]
    ...
    if (container == MP4_CONTAINER)
    {
        ...
        mp4config_enc.samplerate = infile->samplerate;
        mp4config_enc.channels = infile->channels;
        mp4config_enc.bits = infile->samplebytes * 8;
    }
    else
    {
    ...
    if (container == MP4_CONTAINER)
    {
        ...
        faacEncGetDecoderSpecificInfo(hEncoder,
                                      &mp4config_enc.asc.data,
                                      &mp4config_enc.asc.size);
        strcpy(version_string, "FAAC ");
        strcpy(version_string + 5, faac_id_string);

        mp4config_enc.tag.encoder = version_string;

#define SETTAG(x) if(x)mp4config_enc.tag.x=x
        SETTAG(artist);
        SETTAG(artistsort);
        SETTAG(composer);
        SETTAG(composersort);
        ...
        if (artData && artSize)
        {
            mp4config_enc.tag.cover.data = artData;
            mp4config_enc.tag.cover.size = artSize;
        }
...
//JNIEXPORT void JNICALL Java_com_example_overo_audio_codec_AACEncoder_encode(JNIEnv *env, jobject thiz, jstring in_file_path, jstring out_file_path)
//{
//
//    char* argv[15] = {
//            "faac",
...
//    int argc = 15;
//
//    argv[1] = (*env)->GetStringUTFChars(env, in_file_path, 0);
//    argv[argc - 1] = (*env)->GetStringUTFChars(env, out_file_path, 0);
//
//    AACEncode(argc, argv);
//

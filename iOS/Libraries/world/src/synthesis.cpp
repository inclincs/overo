...
void synthesize(const struct voicefeature* voice_feature, const int sampling_rate, const int y_length, double* y) {
    int fft_size = static_cast<int>(pow(2.0, 1.0 + static_cast<int>(log(3.0 * sampling_rate / world::kFloorF0 + 1) / world::kLog2)));
    
    double frame_period = 5.0;
    
    Synthesis(voice_feature->f0,
              voice_feature->f0_length,
              voice_feature->spectrogram,
              voice_feature->aperiodicity,
              fft_size,
              frame_period,
              sampling_rate,
              y_length,
...
double* synthesis(double* f0, int f0_length, double** sp, double** ap, int fs) {
    int fft_size = static_cast<int>(pow(2.0, 1.0 + static_cast<int>(log(3.0 * fs / world::kFloorF0 + 1) / world::kLog2)));
    ...
    Synthesis(f0, f0_length, sp, ap, fft_size, frame_period, fs, y_length, y);

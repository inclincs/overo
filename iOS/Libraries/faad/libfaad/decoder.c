...
{
    static char *libfaadName = "ios_package";
                              ...
                              unsigned char *channels)
{
    ...
    *channels = 1;
    ...
    if (*channels == 1)
    {
        // [DONT_UPSAMPLING] previous setting : *channels = 2  
        *channels = 1;
    }
#endif
...
static void create_channel_config(NeAACDecStruct *hDecoder, NeAACDecFrameInfo *hInfo)
{
    ...
    if (hDecoder->pce_set)
    {
        ...
        if (chdir & 1)
        {
            ...
            // [DONT_UPSAMPLING] previous setting : as follows
                              ...
                              unsigned long sample_buffer_size)
{
    ...
    {
        /* upMatrix to 2 channels for implicit signalling of PS */
        // [DONT_UPSAMPLING] previous setting : hDecoder->upMatrix = 1;
        hDecoder->upMatrix = 1;
        // [DONT_UPSAMPLING] previous setting : output_channels = 2
        output_channels = 1;
    }
#endif
...
// [OVERO] - start
void NeAACDecInitForOvero(NeAACDecHandle hpDecoder, int overo_mode, char *sbi_file_path, char *olp_file_path)
{
    NeAACDecStruct* hDecoder = (NeAACDecStruct*)hpDecoder;
    Overo *pOvero = &hDecoder->overo;

    pOvero->mode = overo_mode;

    if (overo_mode == 1)
    {
        // For Postprocessing
        loadSpeechPrivacySensitiveBlockIndices(pOvero, sbi_file_path);

        pOvero->olp_file = fopen(olp_file_path, "wb");
    } else if (overo_mode == 2)
    {
        // For Verification
        pOvero->olp_file = fopen(olp_file_path, "rb");
    }
}

unsigned char loadSpeechPrivacySensitiveBlockIndices(OveroHandle hpOvero, char *sbi_file_path) {
    // .sbi를 읽고 각 영역의 마지막 Speech Privacy Sensitive Block의 Index를 저장함
    ...
    // 2. 설명:
    // range := (start block index: String),(end block index: String)
    
    Overo *pOvero = (Overo*) hpOvero;

    FILE* sbi_file = fopen(sbi_file_path, "r");
    if (sbi_file == NULL) {
        printf("Not found .sbi file, please check .sbi file name\n");
        return 1;
    }

    char line[255];

    char* sensitive_block_range_count_string = fgets(line, sizeof(line), sbi_file);
    int sensitive_block_range_count = atoi(sensitive_block_range_count_string);

    pOvero->sensitive_block_range_count = sensitive_block_range_count;
    pOvero->sensitive_block_indices_in_each_range = (uint32_t*) malloc (sizeof(uint32_t *) * sensitive_block_range_count);

    for (int i = 0; i < sensitive_block_range_count; i++) {
        fgets(line, sizeof(line), sbi_file);
        
        strtok(line, ","); // char *start_frame_index_string
        char *end_frame_index_string = strtok(NULL, ",");
        int end_frame_index = atoi(end_frame_index_string);

        pOvero->sensitive_block_indices_in_each_range[i] = (uint32_t) end_frame_index;
    }

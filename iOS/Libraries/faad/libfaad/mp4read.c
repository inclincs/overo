...
static int ftypin(int size)
{
    ...
    if (mp4config_dec.verbose.header)
...
static int mdhdin(int size)
{
    ...
    u32in();
    // Creation time
    mp4config_dec.ctime = u32in();
    // Modification time
    mp4config_dec.mtime = u32in();
    // Time scale
    mp4config_dec.samplerate = u32in();
    // Duration
    mp4config_dec.samples = u32in();
    // Language
    u16in();
    // pre_defined
    u16in();
...
static int mp4ain(int size)
{
    ...
    u32in();
    // Number of channels
    mp4config_dec.channels = u16in();
    // Sample size (bits)
    mp4config_dec.bits = u16in();
    // Compression ID
    u16in();
    // Packet size
    u16in();
...
static int esdsin(int size)
{
    ...
    u8in();
    // buffer size (24 bits)
    mp4config_dec.buffersize = u16in() << 8;
    mp4config_dec.buffersize |= u8in();
    // bitrate
    mp4config_dec.bitratemax = u32in();
    mp4config_dec.bitrateavg = u32in();

    if (u8in() != TAG_DSI)
        return ERR_FAIL;
    mp4config_dec.asc.size = getsize();
    if (mp4config_dec.asc.size > sizeof(mp4config_dec.asc.buf))
        return ERR_FAIL;
    // get AudioSpecificConfig
    datain(mp4config_dec.asc.buf, mp4config_dec.asc.size);
...
static int stszin(int size)
{
    ...
    u32in();
    // Number of entries
    mp4config_dec.frame.ents = u32in();

    if (!(mp4config_dec.frame.ents + 1))
        return ERR_FAIL;

    mp4config_dec.frame.data = malloc(sizeof(*mp4config_dec.frame.data)
                                  * (mp4config_dec.frame.ents + 1));

    if (!mp4config_dec.frame.data)
        return ERR_FAIL;

    ofs = 0;
    mp4config_dec.frame.data[0] = ofs;
    for (cnt = 0; cnt < mp4config_dec.frame.ents; cnt++)
    {
        uint32_t fsize = u32in();

        ofs += fsize;
        if (mp4config_dec.frame.maxsize < fsize)
            mp4config_dec.frame.maxsize = fsize;

        mp4config_dec.frame.data[cnt + 1] = ofs;

        if (ofs < mp4config_dec.frame.data[cnt])
            return ERR_FAIL;
    }
...
{
    if (mp4config_dec.frame.current >= mp4config_dec.frame.ents)
        return ERR_FAIL;

    mp4config_dec.bitbuf.size = mp4config_dec.frame.data[mp4config_dec.frame.current + 1]
        - mp4config_dec.frame.data[mp4config_dec.frame.current];

    if (fread(mp4config_dec.bitbuf.data, 1, mp4config_dec.bitbuf.size, g_fin)
        != mp4config_dec.bitbuf.size)
    {
        fprintf(stderr, "can't read frame data(frame %d@0x%x)\n",
               mp4config_dec.frame.current,
               mp4config_dec.frame.data[mp4config_dec.frame.current]);
...
{
    if (framenum > mp4config_dec.frame.ents)
        return ERR_FAIL;
    if (fseek(g_fin, mp4config_dec.mdatofs + mp4config_dec.frame.data[framenum], SEEK_SET))
        return ERR_FAIL;

    mp4config_dec.frame.current = framenum;
...
{
    fprintf(stderr, "Modification Time:\t\t%s\n", mp4time(mp4config_dec.mtime));
    fprintf(stderr, "Samplerate:\t\t%d\n", mp4config_dec.samplerate);
    fprintf(stderr, "Total samples:\t\t%d\n", mp4config_dec.samples);
    fprintf(stderr, "Total channels:\t\t%d\n", mp4config_dec.channels);
    fprintf(stderr, "Bits per sample:\t%d\n", mp4config_dec.bits);
    fprintf(stderr, "Buffer size:\t\t%d\n", mp4config_dec.buffersize);
    fprintf(stderr, "Max bitrate:\t\t%d\n", mp4config_dec.bitratemax);
    fprintf(stderr, "Average bitrate:\t%d\n", mp4config_dec.bitrateavg);
    fprintf(stderr, "Samples per frame:\t%d\n", mp4config_dec.framesamples);
    fprintf(stderr, "Frames:\t\t\t%d\n", mp4config_dec.frame.ents);
    fprintf(stderr, "ASC size:\t\t%d\n", mp4config_dec.asc.size);
    fprintf(stderr, "Duration:\t\t%.1f sec\n", (float)mp4config_dec.samples/mp4config_dec.samplerate);
    fprintf(stderr, "Data offset/size:\t%x/%x\n", mp4config_dec.mdatofs, mp4config_dec.mdatsize);
}

int mp4read_close(void)
{
#define FREE(x) if(x){free(x);x=0;}
    FREE(mp4config_dec.frame.data);
    FREE(mp4config_dec.bitbuf.data);
...
int mp4read_open(char *name)
{
    ...
    // alloc frame buffer
    mp4config_dec.bitbuf.data = malloc(mp4config_dec.frame.maxsize);

    if (!mp4config_dec.bitbuf.data)
        goto err;

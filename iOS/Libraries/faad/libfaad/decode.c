...
static int decodeAACfile(char *aacfile, char *sndfile, char *adts_fn, int to_stdout,
                  ...
                  float *song_length, int overo_mode, char* sbi_file_path, char* olp_file_path)
{
    int tagsize;
    ...
    if (overo_mode != 0)
    {
        NeAACDecInitForOvero(hDecoder, overo_mode, sbi_file_path, olp_file_path);
    }
    // [OVERO] - end
    ...
    // [DONT_UPSAMPLING] previous setting : config->dontUpSampleImplicitSBR = 0;
    ...
    *song_length = length;
    ...
    do
    {
        ...
        if (first_time && !frameInfo.error)
        {
            ...
            if (!adts_out)
            {
                ...
                if (!to_stdout)
                {
                    ...
                    aufile = open_audio_file(overo_mode == 1 ? "=" : "-", frameInfo.samplerate, frameInfo.channels,
                        outputFormat, fileType, aacChannelConfig2wavexChannelMask(&frameInfo));
                }
        ...
        if ((frameInfo.error == 0) && (frameInfo.samples > 0) && (!adts_out) && overo_mode != 1)
        {
            if (write_audio_file(aufile, sample_buffer, (int) frameInfo.samples, 0) == 0)
                break;
        }
                  ...
                  int infoOnly, int adts_out, float *song_length, float seek_to)
{
    ...
    if (!quiet)
    {
        mp4config_dec.verbose.header = 1;
        mp4config_dec.verbose.tags = 1;
    }
    ...
    if(NeAACDecInit2(hDecoder, mp4config_dec.asc.buf, mp4config_dec.asc.size,
    ...
    if (mp4config_dec.asc.size)
    {
        if (NeAACDecAudioSpecificConfig(mp4config_dec.asc.buf, mp4config_dec.asc.size, &mp4ASC) >= 0)
        {
    ...
    faad_fprintf(stderr, "%s file info:\n\n", mp4file);
    {
        ...
        float seconds;
        seconds = (float)mp4config_dec.samples/(float)mp4ASC.samplingFrequency;
    ...
    startSampleId = 0;
    if (seek_to > 0.1)
        startSampleId = (int64_t)(seek_to * mp4config_dec.samplerate / framesize);

    mp4read_seek(startSampleId);
    for (sampleId = startSampleId; sampleId < mp4config_dec.frame.ents; sampleId++)
    {
        /*int rc;*/
        long dur;
        ...
        sample_buffer = NeAACDecDecode(hDecoder, &frameInfo, mp4config_dec.bitbuf.data, mp4config_dec.bitbuf.size);
        ...
        if (adts_out == 1)
        {
            ...
            fwrite(mp4config_dec.bitbuf.data, 1, frameInfo.bytesconsumed, adtsFile);
        }

        dur = frameInfo.samples / frameInfo.channels;
        decoded += dur;

        if (decoded > mp4config_dec.samples)
            dur += mp4config_dec.samples - decoded;
        ...
        if (!noGapless)
        {
            if (useAacLength || (mp4config_dec.samplerate != samplerate)) {
        ...
        percent = min((int)(sampleId*100)/mp4config_dec.frame.ents, 100);
        if (percent > old_percent)
        {
            old_percent = percent;
...
int faad_main(int argc, char *argv[])
{
    ...
    // [OVERO] - start
    int overo_mode = 0;
    char* sbi_file_path = NULL;
    char* olp_file_path = NULL;
    // [OVERO] - end
    ...
    while (1) {
        ...
        case 'r':
            if (optarg)
            {
                ...
                sbi_file_path = (char *) malloc(sizeof(char) * (strlen(argv[optind]) + 1));
                if (sbi_file_path == NULL) {
                    faad_fprintf(stderr, "Error: Allocating memory for sbi file path\n");
                    return 1;
                }
                strncpy(sbi_file_path, argv[optind++], 256);

                if (optind >= argc || *argv[optind] == '-') {
                    faad_fprintf(stderr, "2 arguments(.sbi, .olp) required for extraction.\n");
                    return 1;
                }

                olp_file_path = (char *) malloc(sizeof(char) * (strlen(argv[optind]) + 1));
                if (olp_file_path == NULL) {
                    faad_fprintf(stderr, "Error: Allocating memory for olp file path\n");
                    return 1;
                }
                strcpy(olp_file_path, argv[optind++]);
...
#undef BUILD_DATE
    ...
    if (readFromStdin == 1) {
        ...
        result = decodeAACfile(aacFileName, audioFileName, adtsFileName, writeToStdio,
            ...
            old_format, &length, overo_mode, sbi_file_path, olp_file_path);
    }
...
int AACDecode(int argc, char *argv[])
{
...
//JNIEXPORT void JNICALL Java_com_example_overo_audio_codec_AACDecoder_decode(JNIEnv *env, jobject thiz, jstring in_file_path, jstring out_file_path){
//    char* argv[4] = {
//            "faad",
//            "-o",
//            NULL,
...
//    int argc = 4;
//
//    argv[argc - 2] = (*env)->GetStringUTFChars(env, out_file_path, 0);
//    argv[argc - 1] = (*env)->GetStringUTFChars(env, in_file_path, 0);
//
//    int res = AACDecode(argc, argv);
//
//    //[OVERO] for multilple uses of getopt
//    optind = 0;
//}
